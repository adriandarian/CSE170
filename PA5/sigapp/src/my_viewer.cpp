
# include "my_viewer.h"

# include <sigogl/ui_button.h>
# include <sigogl/ui_radio_button.h>
# include <sig/sn_primitive.h>
# include <sig/sn_transform.h>
# include <sig/sn_manipulator.h>

# include <sigogl/ws_run.h>

bool tSmooth;

float R = 1.0f;
float r = 0.2f;
int n = 10;

double th;
double ph;
double pi = 3.14159f;

int pCount = 1;


MyViewer::MyViewer ( int x, int y, int w, int h, const char* l ) : WsViewer(x,y,w,h,l)
{
	R = 1.0f;
	r = 0.2f;
	n = 10;
	_nbut=0;
	_animating=false;
	build_ui ();
	build_scene ();
}

void MyViewer::build_ui ()
{
	UiPanel *p, *sp;
	UiManager* uim = WsWindow::uim();
	p = uim->add_panel ( "", UiPanel::HorizLeft );
	p->add ( new UiButton ( "View", sp=new UiPanel() ) );
	{	UiPanel* p=sp;
		p->add ( _nbut=new UiCheckButton ( "Normals", EvNormals ) ); 
	}
	p->add ( new UiButton ( "Animate", EvAnimate ) );
	p->add ( new UiButton ( "Exit", EvExit ) ); p->top()->separate();
}

void MyViewer::add_model ( SnShape* s, GsVec p )
{
	// This method demonstrates how to add some elements to our scene graph: lines,
	// and a shape, and all in a group under a SnManipulator.
	// Therefore we are also demonstrating the use of a manipulator to allow the user to
	// change the position of the object with the mouse. If you do not need mouse interaction,
	// you can just use a SnTransform to apply a transformation instead of a SnManipulator.
	// You would then add the transform as 1st element of the group, and set g->separator(true).
	// Your scene graph should always be carefully designed according to your application needs.

	SnManipulator* manip = new SnManipulator;
	GsMat m;
	m.translation ( p );
	manip->initial_mat ( m );

	SnGroup* g = new SnGroup;
	SnLines* l = new SnLines;
	l->color(GsColor::orange);
	g->add(s);
	g->add(l);
	manip->child(g);
	// manip->visible(false); // call this to turn off mouse interaction

	rootg()->add(manip);
}

void MyViewer::build_scene ()
{
	
	SnModel* sn = new SnModel;
	GsModel& myTorus = *sn->model();
	rootg()->add(sn);

	double dth = pi / n;
	int point0 = 0;
	int point1 = 1;
	int point2 = 2;
	int point3 = 3;

	for (th = 0.0; th < 2 * pi; th += dth) {
		for (ph = 0.0; ph < 2 * pi; ph += dth) {
			//Peset locations for the 6 points based on Torus formulas
			GsVec p1((R + r * cos(th)) * cos(ph), (R + r * cos(th)) * sin(ph), r * sin(th));
			GsVec p2((R + r * cos(th + dth)) * cos(ph), (R + r * cos(th + dth)) * sin(ph), r * sin(th + dth));
			GsVec p3((R + r * cos(th)) * cos(ph + dth), (R + r * cos(th)) * sin(ph + dth), r * sin(th));
			GsVec p4((R + r * cos(th + dth)) * cos(ph + dth), (R + r * cos(th + dth)) * sin(ph + dth), r * sin(th + dth));


			//First triangle
			myTorus.V.push() = p1;
			myTorus.V.push() = p2;
			myTorus.V.push() = p3;
			myTorus.V.push() = p4;

			GsModel::Face f1 = GsModel::Face(point0, point2, point3);
			GsModel::Face f2 = GsModel::Face(point0, point3, point1);

			myTorus.F.push() = f1;
			myTorus.F.push() = f2;
			
			if (!tSmooth) {

				GsVec org((R * cos(ph)), (R * sin(ph)), (double)0);
				GsVec org1((R * cos(ph + dth)), (R * sin(ph + dth)), (double)0);

				GsVec newp1 = (p1 - org);
				GsVec newp2 = (p2 - org);
				GsVec newp3 = (p3 - org1);
				GsVec newp4 = (p4 - org1);

				myTorus.N.push() = newp1;
				myTorus.N.push() = newp2;
				myTorus.N.push() = newp3;
				myTorus.N.push() = newp4;

				myTorus.set_mode(GsModel::Smooth, GsModel::NoMtl);
			}
			else {

				GsPnt newx;
				newx.cross(p3 - p1, p4 - p3);
				GsPnt newy;
				newy.cross(p4 - p1, p2 - p4);

				myTorus.N.push() = newx;
				myTorus.N.push() = newy;

				myTorus.set_mode(GsModel::Flat, GsModel::NoMtl);
			}
			point0 += 4;
			point1 += 4;
			point2 += 4;
			point3 += 4;
		}
	}
	GsModel::Group& g = *myTorus.G.push();
	g.fi = 0;
	g.fn = myTorus.F.size();
	g.dmap = new GsModel::Texture;
	if (pCount == 1) {
		g.dmap->fname.set("..\\src\\flower.png");
	}
	else if (pCount == 2) {
		g.dmap->fname.set("..\\src\\splat.png");
	}
	else if (pCount == 3) {
		g.dmap->fname.set("..\\src\\clouds.jpg");
	}

	myTorus.M.push().init();
	int nv = myTorus.V.size();
	myTorus.T.size(nv);

	int i = 0;
		for (th = 0.0; th < 2 * pi; th += dth) {
			for (ph = 0.0; ph < 2 * pi; ph += dth) {
				myTorus.T[i].set(th / (2 * pi), ph / (2 * pi));
				myTorus.T[i + 1].set((th + dth) / (2 * pi), ph / (2 * pi));
				myTorus.T[i + 2].set(th / (2 * pi), (ph + dth) / (2 * pi));
				myTorus.T[i + 3].set((th + dth) / (2 * pi), (ph + dth) / (2 * pi));

				i += 4;
			}
		}

	myTorus.set_mode(GsModel::Smooth, GsModel::PerGroupMtl);
	myTorus.textured = true;
}
void increment(){}
void MyViewer::drawAgain() {
	rootg()->remove_all();
	build_scene();
	render();
}

// Below is an example of how to control the main loop of an animation:
void MyViewer::run_animation ()
{
	if ( _animating ) return; // avoid recursive calls
	_animating = true;
	
	int ind = gs_random ( 0, rootg()->size()-1 ); // pick one child
	SnManipulator* manip = rootg()->get<SnManipulator>(ind); // access one of the manipulators
	GsMat m = manip->mat();

	double frdt = 1.0/30.0; // delta time to reach given number of frames per second
	double v = 4; // target velocity is 1 unit per second
	double t=0, lt=0, t0=gs_time();
	do // run for a while:
	{	while ( t-lt<frdt ) { ws_check(); t=gs_time()-t0; } // wait until it is time for next frame
		double yinc = (t-lt)*v;
		if ( t>2 ) yinc=-yinc; // after 2 secs: go down
		lt = t;
		m.e24 += (float)yinc;
		if ( m.e24<0 ) m.e24=0; // make sure it does not go below 0
		manip->initial_mat ( m );
		render(); // notify it needs redraw
		ws_check(); // redraw now
	}	while ( m.e24>0 );
	_animating = false;
}

void MyViewer::show_normals ( bool view )
{
	// Note that primitives are only converted to meshes in GsModel
	// at the first draw call.
	GsArray<GsVec> fn;
	SnGroup* r = (SnGroup*)root();
	for ( int k=0; k<r->size(); k++ )
	{	SnManipulator* manip = r->get<SnManipulator>(k);
		SnShape* s = manip->child<SnGroup>()->get<SnShape>(0);
		SnLines* l = manip->child<SnGroup>()->get<SnLines>(1);
		if ( !view ) { l->visible(false); continue; }
		l->visible ( true );
		if ( !l->empty() ) continue; // build only once
		l->init();
		if ( s->instance_name()==SnPrimitive::class_name )
		{	GsModel& m = *((SnModel*)s)->model();
			m.get_normals_per_face ( fn );
			const GsVec* n = fn.pt();
			float f = 0.33f;
			for ( int i=0; i<m.F.size(); i++ )
			{	const GsVec& a=m.V[m.F[i].a]; l->push ( a, a+(*n++)*f );
				const GsVec& b=m.V[m.F[i].b]; l->push ( b, b+(*n++)*f );
				const GsVec& c=m.V[m.F[i].c]; l->push ( c, c+(*n++)*f );
			}
		}  
	}
}

int MyViewer::handle_keyboard ( const GsEvent &e )
{
	int ret = WsViewer::handle_keyboard ( e ); // 1st let system check events
	if ( ret ) return ret;

	switch ( e.key )
	{	case GsEvent::KeyEsc : gs_exit(); return 1;
		case 'n' : { bool b=!_nbut->value(); _nbut->value(b); show_normals(b); return 1; }
		default: gsout<<"Key pressed: "<<e.key<<gsnl;

		case 'q':
			n += 1;
			drawAgain();
			// gsout << n << gsnl;
			return 1;

		case 'a':
			if (n >= 4)
				n -= 1;
			drawAgain();
			// gsout << n << gsnl;
			return 1;

		case 'w':
			r += 0.1f;
			drawAgain();
			// gsout << r << gsnl;
			return 1;

		case 's':
			r -= 0.1f;
			drawAgain();
			// gsout << r << gsnl;
			return 1;

		case 'e':
			R += 0.1f;
			drawAgain();
			// gsout << R << gsnl;
			return 1;
			
		case 'd':
			R -= 0.1f;
			drawAgain();
			// gsout << R << gsnl;
			return 1;

		case 'z':
			tSmooth = true;
			drawAgain();
			return 1;

		case 'x':
			tSmooth = false;
			drawAgain();
			return 1;
		
		case ' ':
			pCount++;
			if (pCount == 4) {
				pCount = 1;
			}
			drawAgain();
			return 1;
	}

	return 0;
}

int MyViewer::uievent ( int e )
{
	switch ( e )
	{	case EvNormals: show_normals(_nbut->value()); return 1;
		case EvAnimate: run_animation(); return 1;
		case EvExit: gs_exit();
	}
	return WsViewer::uievent(e);
}
